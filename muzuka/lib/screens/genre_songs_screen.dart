...
class GenreSongsScreen extends ConsumerStatefulWidget {
  ...
  const GenreSongsScreen({super.key, required this.genreId, required this.genreName});

  @override
  ConsumerState<GenreSongsScreen> createState() => _GenreSongsScreenState();
}
  ...
  Future<void> _loadSongs() async {
    try {
      final response = await apiClient.get<Map<String, dynamic>>(
        '${ApiConstants.genreSongs(widget.genreId)}?page=$_page&limit=20',
        fromJson: (json) => json,
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _songs.isEmpty
              ? const Center(
                  child: Text('No songs in this genre', style: TextStyle(color: AppColors.textTertiary)),
                )
              : ListView.builder(
