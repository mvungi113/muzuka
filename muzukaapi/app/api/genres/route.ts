...
export async function GET(request: NextRequest) {
  try {
    ...
    return Response.json(paginatedResponse(genres, page, limit, total));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const slug = await uniqueSlug(parsed.data.name, async (s) => {
      const existing = await prisma.genre.findUnique({ where: { slug: s } });
      return !!existing;
    });

    const genre = await prisma.genre.create({ data: { ...parsed.data, slug } });
    return Response.json(successResponse(genre), { status: 201 });
  } catch (error) {
